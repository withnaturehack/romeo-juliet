...
export default function PoliticsPage() {
  const sectionContent = ONBOARDING_STEP_2_SECTIONS.politics;
  ...
  const [comfortWithDifferences, setComfortWithDifferences] = useState("");

  const comfortRef = useRef<HTMLElement>(null);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase
        .from("profiles")
        .select("political_and_social_outlook")
        .eq("user_id", session.user.id)
        .limit(1);
      const saved = data?.[0]?.political_and_social_outlook;
      if (saved) {
        setPoliticsImportance(saved.politics_importance ?? "");
        setComfortWithDifferences(saved.comfort_with_different_views ?? "");
      }
  ...
  const handleContinue = async () => {
    ...
    if (!politicsImportance) {
      setError(sectionContent.errors.politicsImportance);
      return;
    }
    if (!comfortWithDifferences) {
      setError(sectionContent.errors.comfortWithDifferences);
      return;
    }
    ...
    try {
      const sectionPayload = {
        ...
        comfort_with_different_views: comfortWithDifferences || null,
      };

      await saveProfileSection(userId, "political_and_social_outlook", sectionPayload);
      router.replace("/onboarding/step-2/physical");
    } catch (err: unknown) {
    ...
    >
      ...
      <main className="w-full max-w-md mx-auto flex-grow space-y-10 pb-32">
        ...
        <section className="space-y-4">
          ...
          >
            {sectionContent.fields.politicsImportance.heading}
          </h2>
          <div className="space-y-3">
            {sectionContent.fields.politicsImportance.options.map((opt) => (
              <div key={opt.value} className="flex items-center">
                <input
                  type="radio"
                  name="politics-importance"
                  id={`politics-importance-${opt.value}`}
                  value={opt.value}
                  checked={politicsImportance === opt.value}
                  onChange={() => { setPoliticsImportance(opt.value); scrollTo(comfortRef); }}
                  className="hidden"
                />
                <label
                  htmlFor={`politics-importance-${opt.value}`}
                  className="flex items-center cursor-pointer transition-colors duration-300 text-white w-full"
                  style={{ fontFamily: "var(--font-inter), sans-serif" }}
                >
                  <span className={`w-4 h-4 rounded-full border border-white mr-4 shrink-0 transition-all duration-300 ${politicsImportance === opt.value ? 'bg-[#F8EDE3] border-[#F8EDE3]' : 'bg-transparent'}`} />
        ...
        {/* Comfort with Different Views */}
        <section ref={comfortRef} className="space-y-4">
          <h2
            className="text-lg tracking-wide border-b border-white/10 pb-2 text-white"
            style={{ fontFamily: "var(--font-playfair), serif" }}
          >
            {sectionContent.fields.comfortWithDifferences.heading}
          </h2>
          <div className="space-y-3">
            {sectionContent.fields.comfortWithDifferences.options.map((opt) => (
              <div key={opt.value} className="flex items-center">
                <input
                  type="radio"
                  name="comfort-differences"
                  id={`comfort-differences-${opt.value}`}
                  value={opt.value}
                  checked={comfortWithDifferences === opt.value}
                  onChange={() => setComfortWithDifferences(opt.value)}
                  className="hidden"
                />
                <label
                  htmlFor={`comfort-differences-${opt.value}`}
                  className="flex items-center cursor-pointer transition-colors duration-300 text-white w-full"
                  style={{ fontFamily: "var(--font-inter), sans-serif" }}
                >
                  <span className={`w-4 h-4 rounded-full border border-white mr-4 shrink-0 transition-all duration-300 ${comfortWithDifferences === opt.value ? 'bg-[#F8EDE3] border-[#F8EDE3]' : 'bg-transparent'}`} />
                  <span className="text-sm font-light tracking-widest uppercase">{opt.label}</span>
                </label>
              </div>
            ))}
